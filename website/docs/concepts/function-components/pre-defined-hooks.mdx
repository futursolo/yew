...
If you need the component to be re-rendered on state change,
consider using [`use_state`](#use_state) to trigger a render.

This hook returns a special `UseRefHandle<T>` type that provides an API
similar to `std::thread::LocalKey`. This ensures that references are released
at the end of the closure and cannot be carried for an indefinite amount of
time.

Mutable references can only be acquired when no component is rendering.
(Outside of function component body.) If you attempt to acquire a mutable
reference when any component is rendering, your application will panic.
